    ...
    public class CustomerManagerRepository : ICustomerManagerRepository
    {
        ...
        public async Task<IEnumerable<CustomerSearchDto>> GetCustomerForInvoice(string customerName)
        {
            var customers = await _tcContext.Customers.
               Where(x => (EF.Functions.Like(x.Name, "%" + customerName + "%") && x.Deleted == false &&( x.IsSeparateInvoice==true || x.AccountType=="M" ))).ToListAsync();           
        ...
        {
            var customers = await _tcContext.Customers.
               Where(x => (EF.Functions.Like(x.CustomerCode, "%" + customerCode + "%") && x.Deleted == false && (x.IsSeparateInvoice == true || x.AccountType == "M" ))).ToListAsync();
